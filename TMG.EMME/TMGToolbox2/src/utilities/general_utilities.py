...
@contextmanager
def tempMatricesMANAGER(
    number_of_matrices, description="[No description]", matrix_type="FULL", default=0.0
):
    """
    Creates a temporary matrix in a context manager.

    Args:
        - number_of_matrices: number of matrices to create
        - description (="[No description]"): The description of the temporary matrix.
        - matrix_type (='FULL'): The type of temporary matrix to create. One of
            'SCALAR', 'ORIGIN', 'DESTINATION', or 'FULL'.
        - default (=0.0): The matrix's default value.
    ...
    if len(matrices) != number_of_matrices:
        for mtx in matrices:
            _DATABANK.delete_matrix(mtx.id)
        raise Exception("Could not create all temporary matrices: %s" % description)
    ...
    try:
        ...
        s = "Deleted matrices %s." % ", ".join([x.id for x in matrices])
        _m.logbook_write(s)
