...
class Rectangle:
    ...
    def intersects(self, otherRectangle):
        ...
        if "rangeX" in props and "rangeY" in props:
            return self.rangeX.overlaps(otherRectangle.rangeX) and self.rangeY.overlaps(otherRectangle.rangeY)
...
class GridIndex:
    ...
    def __init__(self, extents, xSize=100, ySize=100, marginSize=0.0):
        ...
        if hasattr(extents, "__iter__"):
            ...
            self.extents = Rectangle(minx - marginSize, miny - marginSize, maxx + marginSize, maxy + marginSize)
        else:
    ...
    def nearestToPoint(self, x, y):
        ...
        while (
            ((originCol - i) > 0)
            and ((originRow - i) > 0)
            and ((originCol + i) <= self.maxCol)
            and ((originRow + i) <= self.maxRow)
        ):
...
def find_nearest(candidates, x, y):
    ...
    for candidate in candidates:
        ...
        # if transit segment:
            ...
            shape = _geo.LineString(
                [(candidate.i_node.x, candidate.i_node.y), (candidate.j_node.x, candidate.j_node.y)]
            )
        # if shapely shape:
        else:
            shape = candidate
