...
class Shapely2ESRI:
    ...
    def _createShapefileIfNotExisting(self, geometry):
        if self._sf == None:
            ...
            for field in self._fields.values():
                field.addToDf(self)
            self._sf = fiona.open(
                self._shape_file_path,
                "w",
                "ESRI Shapefile",
                {"geometry": geoType, "properties": self.schema},
            )

    def writeTo(self, geometry, fid, attributes={}):
        self._createShapefileIfNotExisting(geometry)
        properties = geometry.getAttributes()
        self._sf.write({"geometry": mapping(geometry), "properties": properties})
    ...
    def setProjection(self, projectionFile=None):
        ...
        """
        major, minor, release, beta = _util.get_emme_version(tuple)
