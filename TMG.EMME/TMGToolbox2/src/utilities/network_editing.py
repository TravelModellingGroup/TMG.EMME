...
def splitLink(
    ...
    twoWay=True,
    onLink=True,
    coordFactor=COORD_FACTOR,
    stopOnNewNodes=False,
):
    """
...
def copyNetwork(network_to_copy):
    ...
    for etype in element_types:
        ...
        for attname in new_atts - standard_atts:
            new_network.create_attribute(etype, attname)

    # 2. Copy the modes
    for mode_to_copy in network_to_copy.modes():
        new_mode = new_network.create_mode(mode_to_copy.type, mode_to_copy.id)
        for attname in new_network.attributes("MODE"):
            new_mode[attname] = mode_to_copy[attname]
    ...
    for node_to_copy in network_to_copy.nodes():
        ...
        for attname in new_network.attributes("NODE"):
            new_node[attname] = node_to_copy[attname]
    ...
    for transit_line_to_copy in network_to_copy.transit_lines():
        ...
        new_transit_line = new_network.create_transit_line(
            transit_line_to_copy.id, transit_line_to_copy.vehicle.id, itinerary
        )
        for attname in new_network.attributes("TRANSIT_LINE"):
            new_transit_line[attname] = transit_line_to_copy[attname]

        for segment_to_copy, new_segment in _util.itersync(
            transit_line_to_copy.segments(True), new_transit_line.segments(True)
        ):
            for attname in new_network.attributes("TRANSIT_SEGMENT"):
                new_segment[attname] = segment_to_copy[attname]
...
__SEGMENT_ATTRIBUTE_AGGREGATORS = {
    "dwell_time": __SUM,
        ...
        for segment in link.segments():
            if segment.line.segment(-2).number == segment.number:
                ...
                if nextSegment.link.reverse_link == link:
                    raise InvalidNetworkOperationError(
                        "Cannot delete node %s: It is used as a u-turn point for transit line %s."
                        % (node, segment.line)
                    )
...
class TransitSegmentProxy:
    ...
    __MAP = {
        "allow_boardings": "allowBoardings",
            ...
            # Link is part of a turn
                ...
                for turn in link.outgoing_turns():
                    if turn.penalty_func == 0:
                        continue  # Skip prohibited turns
                    toLink = turn.to_link
                    if not self.link_filter(toLink):
