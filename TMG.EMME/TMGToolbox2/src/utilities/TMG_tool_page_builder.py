...
class Face(_m.Tool()):
    def page(self):
        pb = TmgToolPageBuilder(
            self,
            runnable=False,
            title="TMG ToolPageBuilder",
...
class TmgToolPageBuilder(_m.ToolPageBuilder):
    def __init__(
        self, tool, runnable=True, title="", description="", branding_text="", help_path=None, footer_help_links=None
    ):
        ...
        self.root.__init__(
            ...
            help_path=help_path,
            footer_help_links=footer_help_links,
        )
    ...
    def add_new_scenario_select(
        self, tool_attribute_name="", title="", note="", next_scenario_option=True, allow_none=False
    ):
        ...
        self.root.add_select(
            tool_attribute_name=tool_attribute_name, keyvalues=availableScenarioIds, title=title, note=note
        )

    # -------------------------------------------------------------------------------------------

    def add_select_output_matrix(
        self,
        tool_attribute_name,
        matrix_types=["FULL"],
        title="",
        ...
        if include_next:
            for matrixType in matrix_types:
                ...
                options.append((nextId, "Next available %s matrix" % matrixType))
            ...
            for matrixType in matrix_types:
                ...
                for i in range(1, dimension + 1):
                    ...
                    if matrix == None and include_new:
                        ...
                        options.append((id, _getMatrixText(matrix)))
        if len(options) == 0:
            raise TypeError("No options were permitted")

        self.root.add_select(tool_attribute_name, keyvalues=options, title=title, note=note, searchable=True)

    # -------------------------------------------------------------------------------------------

    # @deprecated: Use add_select_output_matrix instead
    def add_select_new_matrix(
        self,
        tool_attribute_name,
        matrix_type="FULL",
        title="",
        ...
        self.root.add_select(
            tool_attribute_name=tool_attribute_name,
...
class _table:
    ...
    def __enter__(self):
        ...
        if self.width == None:
            ...
            cellpadding='0'>".format(
                borderWidth, frame
            )
        else:
            s = "<div class='sm_indent'><table class='tmg_table' style='border-style:{1};' border={0} \
            cellpadding='0' width='{2}%'>".format(
                borderWidth, frame, self.width
            )
    ...
    def table_cell(self, **attributes):
        h = "<td"
        for keyval in attributes.items():
            h += " %s='%s'" % keyval
        h += ">"
